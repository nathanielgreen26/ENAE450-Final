...
class WallFollow(Node):
    ...
    def listener_callback(self, msg):
        ...
        if self.start_wall:
            ...
            if self.stage == self.stages.MOVING_FORWARD:
                ...
                start_wall_obstacle = self.check_obstacle(
                    right, self.side_threshold) if self.start_wall == self.walls.RIGHT else self.check_obstacle(left, self.side_threshold)
                ...
                if not front_obstacle:
                    ...
                    if self.check_obstacle(front_left, self.corner_threshold):
                        print("Adjust right")
                        self.turn_right()
                        time.sleep(self.delay)

                    # Turn left to avoid hitting right wall
                    elif self.check_obstacle(front_right, self.corner_threshold):
                        print("Adjust left")
                        self.turn_left()
                        time.sleep(self.delay)
            ...
            # If I am already turning:
                ...
                front_left_obstacle = self.check_obstacle(front_left, self.corner_threshold)
                front_right_obstacle = self.check_obstacle(front_right, self.corner_threshold)
    ...
    def check_obstacle(self, dir: List[float], limit: float):
        count = 0
        for val in dir:
            if 0.0 <= val < self.wall_dist:
                count += 1
                if count > limit:
                    return 1
        return 0
    ...
    def check_count(self, dir: List[float]):
        count = 0
        for val in dir:
            if 0.0 <= val < self.wall_dist:
                count += 1
